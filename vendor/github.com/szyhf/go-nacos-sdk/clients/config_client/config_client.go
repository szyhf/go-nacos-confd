...
import (
	...
	"github.com/szyhf/go-nacos-sdk/clients/cache"
	"github.com/szyhf/go-nacos-sdk/clients/nacos_client"
	"github.com/szyhf/go-nacos-sdk/common/constant"
	"github.com/szyhf/go-nacos-sdk/common/http_agent"
	"github.com/szyhf/go-nacos-sdk/common/logger"
	"github.com/szyhf/go-nacos-sdk/common/nacos_error"
	"github.com/szyhf/go-nacos-sdk/common/util"
...
func (client *ConfigClient) GetConfig(param vo.ConfigParam) (content string, err error) {
	...
	content, err = client.decrypt(param.DataId, content)
	if err != nil {
		fmt.Println(err.Error())
		return "", err
	}
...
func (client *ConfigClient) decrypt(dataId, content string) (string, error) {
	// fmt.Printf("dataId: %s, %v\n", dataId, client.kmsClient == nil)
	switch true {
	case client.kmsClient == nil:
		// 不进行解密流程
		// fmt.Printf("不进行解密")
	case strings.HasPrefix(dataId, "cipher-kms-aes-128-"):
		// 底层魔改：使用\n分隔encryptedKey和cryptedData
		lines := strings.Split(content, "\n")
		// fmt.Printf("AES128: %+v\n", lines)
		if len(lines) == 2 {
			encryptedKey := lines[0]
			request := kms.CreateDecryptRequest()
			request.Method = "POST"
			request.Scheme = "https"
			request.AcceptFormat = "json"
			request.CiphertextBlob = encryptedKey
			response, err := client.kmsClient.Decrypt(request)
			if err != nil {
				// fmt.Println(err.Error())
				return "", errors.New("kms decrypt aes128 key failed: " + err.Error())
			}
			dataKey := response.Plaintext
			dataKeyBytes, err := base64.StdEncoding.DecodeString(dataKey)
			if err != nil {
				return "", errors.New("DataKey解码失败：" + err.Error())
			}
			cryptedData := lines[1]
			...
			dataBytes, err := kmsAES_ECB_PKCS5PaddingDecrypt(cryptedDataBytes, dataKeyBytes)
			if err != nil {
				return "", errors.New("cryptedDataBytes解密失败：" + err.Error())
			}

			// fmt.Println("DataKey: ", dataKey)
			// fmt.Println("EncData: ", lines[0])
			return string(dataBytes), nil
		} else {
			return "", errors.New("kms get aes128 key failed")
		}
	case strings.HasPrefix(dataId, "cipher-"):
		...
		if err != nil {
			// fmt.Println(err.Error())
			return "", errors.New("kms decrypt failed" + err.Error())
		}
